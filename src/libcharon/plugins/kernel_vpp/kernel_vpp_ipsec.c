...
static status_t convert_enc_alg(uint16_t alg, chunk_t key, uint16_t *vpp_alg)
{
    ...
    else if (ENCR_AES_CBC == alg)
    {
        ...
        {
            case 128:
                *vpp_alg = KIKNOS_AES_CBC_128;
                break;
            case 192:
                *vpp_alg = KIKNOS_AES_CBC_192;
                break;
            case 256:
                *vpp_alg = KIKNOS_AES_CBC_256;
                break;
            default:
                return FAILED;
        }
    ...
    else if (ENCR_AES_CTR == alg)
    {
        ...
        {
            case 128:
                *vpp_alg = KIKNOS_AES_CTR_128;
                break;
            case 192:
                *vpp_alg = KIKNOS_AES_CTR_192;
                break;
            case 256:
                *vpp_alg = KIKNOS_AES_CTR_256;
                break;
            default:
                return FAILED;
        }
    ...
    {
        *vpp_alg = KIKNOS_DES_CBC;
    }
    else if (ENCR_3DES == alg)
    {
        *vpp_alg = KIKNOS_DES3_CBC;
    }
    else
    {
        return FAILED;
...
static status_t convert_int_alg(uint16_t alg, uint16_t *vpp_alg)
{
    ...
    {
        case AUTH_UNDEFINED:
            *vpp_alg = KIKNOS_NONE_INTEG;
            break;
        case AUTH_HMAC_MD5_96:
            *vpp_alg = KIKNOS_MD5_96;
            break;
        case AUTH_HMAC_SHA1_96:
            *vpp_alg = KIKNOS_SHA1_96;
            break;
        case AUTH_HMAC_SHA2_256_96:
            *vpp_alg = KIKNOS_SHA_256_96;
            break;
        case AUTH_HMAC_SHA2_256_128:
            *vpp_alg = KIKNOS_SHA_256_128;
            break;
        case AUTH_HMAC_SHA2_384_192:
            *vpp_alg = KIKNOS_SHA_384_192;
            break;
        case AUTH_HMAC_SHA2_512_256:
            *vpp_alg = KIKNOS_SHA_512_256;
            break;
        default:
            return FAILED;
    }
...
{
    kiknos_ipsec_tunnel_t tun;
    memset(&tun, 0, sizeof(tun));

    tun.name = tp->if_name;

    kiknos_rc_t rc = vac->update_tunnel(vac, &tun, FALSE);
    if (KIKNOS_RC_OK != rc)
    {
...
static status_t create_tunnel(tunnel_t *tp)
{
    ...
    tun.name = tp->if_name;
    tun.interface_with_ip = tp->un_if_name;
    tun.esn = tp->esn;
    tun.enable_udp_encap = tp->udp_encap;
    tun.integ_alg = tp->int_alg;
    tun.crypto_alg = tp->enc_alg;
    tun.local_ip = tp->src_addr;
    tun.local_spi = tp->src_spi;
    tun.local_integ_key = tp->src_int_key;
    tun.local_crypto_key = tp->src_enc_key;
    tun.remote_ip = tp->dst_addr;
    tun.remote_spi = tp->dst_spi;
    tun.remote_integ_key = tp->dst_int_key;
    tun.remote_crypto_key = tp->dst_enc_key;

    rc = vac->update_tunnel(vac, &tun, TRUE /* is_add */);
    if (rc != KIKNOS_RC_OK)
    {
        DBG1(DBG_KNL, "kernel_vpp: error communicating with grpc");
        return FAILED;
...
{
    uint16_t vpp_enc_alg = 0;
    uint16_t vpp_int_alg = 0;

    chunk_t enc_key;
    chunk_t int_key;

    char *if_name, *un_if_name = 0;
    tunnel_t *tunnel;
    status_t rc;
    size_t len;
    char buf[64];
